...
export default function AllCategoriesScreen({ navigation, route }) {
  ...
  const [categories, setCategories] = useState([]), [stars, setStars] = useState([]), [loading, setLoading] = useState(true), [progressData, setProgressData] = useState({});
  ...
  useEffect(() => {
    fetch(`${API_URL}get_categories.php`).then(r => r.json()).then(j => {
      if (j.status === "success") {
        setCategories(j.categories);
        j.categories.forEach(cat => {
          fetch(`${API_URL}ver_progreso.php?userId=${userId}&categoria=${cat.id}`)
            .then(res => res.json())
            .then(p => { if (p.status === "success") setProgressData(prev => ({ ...prev, [cat.id]: p.progress })) });
        });
      }
  ...
  return (
    <View style={s.c} key={route?.key}>
      ...
      <ScrollView style={{ flex: 1, paddingTop: insets.top }} contentContainerStyle={s.scroll}>
        <Animatable.Text animation="fadeInUp" delay={200} style={s.t}>¡Explora todas las categorías!</Animatable.Text>
