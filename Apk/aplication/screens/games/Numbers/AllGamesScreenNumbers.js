...
export default function AllGamesScreenNumbers({ route, navigation }) {
  ...
  // ⬅️ Interceptar botón atrás (forma moderna)
  useFocusEffect(() => {
    const onBackPress = () => {
    ...
    return () => subscription.remove(); // ✅ forma moderna
  });
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}get_games.php?category_id=${categoria}`)
      .then(r => r.json())
      .then(j => {
        if (j.status === "success") setGames(j.games.slice(0, 5));
        else console.warn("⚠️ Error al cargar juegos:", j.message);
      })
          ...
          <View style={s.g}>
            {games.map((game, i) => (
              <CategoryCard
                ...
                delay={100 + i * 200}
                onPress={() => {
                  if (!game.ruta) return console.warn("⚠️ Juego sin ruta:", game);
